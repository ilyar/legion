...
...                 Build stub model  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}  AND
...                 Get token from EDI  ${TEST_MD_NAME}  AND
...                 Run EDI undeploy model without version and check    ${TEST_MD_NAME}  AND
...                 Run EDI deploy and check model started  ${TEST_MD_NAME}  ${TEST_MODEL_IMAGE}  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}
Suite Teardown      Run keywords  Delete stub model training  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}  AND
...                 Shell  legionctl --verbose md delete --md ${TEST_MD_NAME}  AND
...
Refresh security tokens
    ...
    ${res}=  Shell  legionctl generate-token --md ${TEST_MD_NAME}
             Should be equal  ${res.rc}  ${0}
...
Deploy fails when memory resource is incorect
    [Setup]         Run EDI undeploy model without version and check    ${TEST_MD_NAME}
...
Invoke. Empty model service url
    ...
    [Teardown]  Login to the edi and edge
    [Setup]     Remove File  ${LOCAL_CONFIG}
    ${res}=  Shell  legionctl --verbose model invoke --md ${TEST_MD_NAME} -p a=1 -p b=2 --jwt "some_token"
             Should not be equal  ${res.rc}  ${0}
...
Invoke. Empty jwt
    ...
    ${res}=  Shell  legionctl --verbose model invoke --md ${TEST_MD_NAME} -p a=1 -p b=2 --model-server-url ${EDGE_URL}
             Should not be equal  ${res.rc}  ${0}
             Should contain       ${res.stderr}  specify model jwt

Invoke. Wrong jwt
    [Documentation]  Fails if jwt is wrong
    [Setup]  StrictShell  legionctl --verbose md create ${TEST_MD_NAME} --image ${TEST_MODEL_IMAGE}
    ${res}=  Shell  legionctl --verbose model invoke --md ${TEST_MD_NAME} -p a=1 -p b=2 --model-server-url ${EDGE_URL} --jwt wrong
             Should not be equal  ${res.rc}  ${0}
...
Invoke. Pass parameters explicitly
    ...
    ${res}=  Shell  legionctl --verbose model invoke --md ${TEST_MD_NAME} -p a=1 -p b=2 --model-server-url ${EDGE_URL} --jwt "${JWT}"
             Should be equal  ${res.rc}  ${0}
