...
...                 Get token from EDI  ${TEST_MODEL_NAME}  AND
...                 Run EDI deploy and check model started  ${TEST_MODEL_NAME}   ${TEST_MODEL_IMAGE}   ${TEST_MODEL_NAME}    ${TEST_MODEL_VERSION}
Suite Teardown      Run Keywords
...                 Delete stub model training  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}  AND
...
Check model API logging without request ID and one chunk
    ...
    ${response}=   Invoke deployed model    ${LEGION_NAMESPACE}  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}  a=${a_value}  b=${b_value}
...
Check model API logging with request ID and one chunk
    ...
    ${response}=   Invoke deployed model    ${LEGION_NAMESPACE}  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}  request_id=${request_id}  a=${a_value}  b=${b_value}
    Validate model API response      ${response}    result=${expected_response}
...
Check model API logging with request ID and many chunks
    ...
    ${response}=   Invoke deployed model    ${LEGION_NAMESPACE}  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}  endpoint=feedback  request_id=${request_id}  str=${TEST_MODEL_ARG_STR}  copies=${TEST_MODEL_ARG_COPIES}
    Validate model API response      ${response}    result=${expected_response}
...
Check model API feedback with request ID
    ...
    ${response}=   Send feedback for deployed model    ${LEGION_NAMESPACE}  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}  ${request_id}  a=${a_value}  b=${b_value}
