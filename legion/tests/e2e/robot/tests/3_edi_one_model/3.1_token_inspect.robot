...
...                               Run EDI undeploy model without version and check    ${TEST_MD_NAME}
Suite Teardown      Run Keywords  Delete stub model training  ${TEST_MODEL_NAME}  ${TEST_MODEL_VERSION}  AND
...                               Run EDI undeploy model without version and check    ${TEST_MD_NAME}
Test Setup          Run EDI deploy and check model started            ${TEST_MD_NAME}   ${TEST_MODEL_IMAGE}   ${TEST_MODEL_NAME}    ${TEST_MODEL_VERSION}
Test Teardown       Run EDI undeploy model without version and check    ${TEST_MD_NAME}
...
Get token from EDI with valid parameters
    ...
    [Setup]   NONE
    [Tags]  edi_token
    &{data} =    Create Dictionary    md_name=${TEST_MD_NAME}
...
Get token from EDI with expiration date set
    ...
    Log           ${expiration_date}
    &{data} =     Create Dictionary    md_name=${TEST_MD_NAME}    expiration_date=${expiration_date}
    Log           ${data}
    &{resp} =     Execute post request    ${EDI_URL}/api/v1/model/token    json_data=${data}  cookies=${DEX_COOKIES}
    Log           ${resp}
    Should not be empty    ${resp}
...
Get token from EDI with too long expiration date set
    ...
    &{data} =     Create Dictionary    md_name=${TEST_MD_NAME}    expiration_date=${expiration_date_str}
    Log           ${data}
    &{resp} =     Execute post request    ${EDI_URL}/api/v1/model/token    json_data=${data}  cookies=${DEX_COOKIES}
    Log           ${resp}
    Should not be empty    ${resp}
...
Get token from EDI without model_name parameter
    ...
    Should be equal as strings   ${items}   ['message', 'Requested field md_name is not set']
...
Get token from EDI with expiration date set
    ...
    Log  ${expiration_date}
    ${res} =                StrictShell                   legionctl --verbose generate-token --expiration-date ${expiration_date} --md ${TEST_MD_NAME}
                            Log  ${res.stdout}
