...
Test model pipeline result
    ...
    ${model_build_status} =      Get model training status  ${md_name}
    Log                  Model build status is ${model_build_status}

    ${model_name}=      Get From Dictionary  ${model_build_status}  modelName
    ${model_version}=   Get From Dictionary  ${model_build_status}  modelVersion
