...
from legion.sdk.clients.route import build_client, ModelRoute, ModelRouteClient, READY_STATE

DEFAULT_WAIT_TIMEOUT = 5

DEFAULT_WIDTH = 120
MD_HEADER = ["Name", "State", "Edge URL", "Model Deployment Targets", "Feedback", "Mirror"]
...
def _common_mr_params(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--model-route-target', '--target', '-t', action='append', type=str, help='')
...
def generate_parsers(main_subparser: argparse._SubParsersAction) -> None:  # pylint: disable=R0915
    ...
    mr_get_parser.add_argument('name', type=str, nargs='?', help='VCS Credential name', default="")
    ...
    mr_delete_parser.add_argument('name', type=str, nargs='?', help='VCS Credential name', default="")
